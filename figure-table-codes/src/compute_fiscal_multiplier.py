...
def get_behavioural_effect(
    B2, tax, D, extension_rate,
    epsilon_t, epsilon_e, permanent=False):
...
def calculate_multiplier(
    ...
    B_p_takeup, B_p_extension = get_behavioural_effect(
        ...
        epsilon_t=epsilon_t, epsilon_e=epsilon_e,
        permanent=True 
    )
    ...
    if return_multiplier:
        return 1+ ((p_perm * (B_p_takeup+B_p_extension) + (1- p_perm) * (B_t_takeup+B_t_extension)) / (p_perm* M_p + (1-p_perm)*M_t))
    else:
...
def compute_fiscal_multiplier():
    ...
    M_p, M_t, B_p_takeup, B_p_extension, B_t_takeup, B_t_extension = calculate_multiplier(
        B1=B1, B2=B2, 
        tax=tax, D=D, p_perm=p_perm, extension_rate=extension_rate,
        epsilon_t=epsilon_t, epsilon_e=epsilon_e,
    ...
    print(f"Behavioural takeup effect permanent: {round(B_p_takeup,2)}, temporary: {round(B_t_takeup,2)}")
    print(f"Behavioural extension effect permanent: {round(B_p_extension,2)}, temporary: {round(B_t_extension,2)}")
    print(f"Mechanical effect permanent: {round(M_p,2)}, temporary: {round(M_t,2)}")
    print(f"Total effect permanent: {round(total_effect_p,2)}, temporary: {round(total_effect_t,2)}")
    print("\n")
    ...
    with open("../out/fiscal_multiplier.tex", "w") as f:
        ...
        f.write(f"Fiscal Multiplier ignoring exit from DI: {round(multiplier_no_exit,4)} \\\\ \n")
        #f.write(f"Fiscal Multiplier with exit but no exit elasticity: {round(multiplier_no_exit_elasticity,4)} \\\\ \n")
        f.write(f"Behavioural takeup effect permanent: {round(B_p_takeup,2)}, temporary: {round(B_t_takeup,2)} \\\\ \n")
        f.write(f"Behavioural extension effect permanent: {round(B_p_extension,2)}, temporary: {round(B_t_extension,2)} \\\\ \n")
        f.write(f"Mechanical effect permanent: {round(M_p,2)}, temporary: {round(M_t,2)} \\\\ \n")
        f.write(f"Total effect permanent: {round(total_effect_p,2)}, temporary: {round(total_effect_t,2)} \\\\ \n")
