...
def run_all():
    ...
    earnings = [[
        ...
        "MEMP_earnings_post1", "MEMP_earnings_post2", "MEMP_earnings_post3", "MEMP_earnings_post4"
    ]]
    emp_annual =[[
        "REGEMP_post1", "REGEMP_post2", "REGEMP_post3", "REGEMP_post4", "REGEMP_post5", 
        "MEMP_post1", "MEMP_post2", "MEMP_post3", "MEMP_post4", "MEMP_post5"
        ]]
    
    for outcome_list, label in zip(
        [main_emp, earnings, emp_annual], 
        ["employment", "appendix_employment_earnings_annual", "appendix_employment_annual"]
        ):
...
if __name__ == "__main__":
    ...
    for lim, relative in zip([(-200,200), (-0.2,0.2)], [False, True]):
        result = func_select_outcome_df(outcome="RTBT_2014", results_csv=r"OUT_FIRSTSTAGE_HETEROGENEITY.csv")
        coefplot(data=result, outcome="RTBT_2014", tick_size=20, label_size=20, xlim=lim, relative=relative, appendix=relative)
        plt.close()

    # Mortality
    result = func_select_outcome_df(outcome="dead_post_6", results_csv=r"OUT_MORTALITY_HETEROGENEITY.csv")
    coefplot(data=result, outcome="dead_post_6", tick_size=20, label_size=20, xlim=(-0.2,0.2))
    plt.close()

    # DI status/exit from DI
    result = func_select_outcome_df(outcome="status_pension_post4", results_csv="OUT_STATUS_HETEROGENEITY.csv")
    coefplot(data=result, outcome="status_pension_post4",  tick_size=20, label_size=20, xlim=(-0.2,0.2))
    plt.close()

    # Main earnings outcomes
    result = func_select_outcome_df(outcome="REGEMP_avg_rec", results_csv="OUT_LABOR_HETEROGENEITY.csv")
    coefplot(data=result, outcome="REGEMP_avg_rec",  tick_size=20, label_size=20, xlim=(-3000,3000))
    plt.close()

    result = func_select_outcome_df(outcome="MEMP_avg_rec", results_csv="OUT_LABOR_HETEROGENEITY.csv")
    coefplot(data=result, outcome="MEMP_avg_rec",  tick_size=20, label_size=20, xlim=(-500,500))
    plt.close()

    # Employment 
    for outcome in "MEMP_rec", "REGEMP_rec":
        result = func_select_outcome_df(outcome=outcome, results_csv="OUT_LABOR_HETEROGENEITY.csv")
        coefplot(data=result, outcome=outcome,  tick_size=20, label_size=20, xlim=(-0.2,0.2), appendix=True)
        plt.close()

    for outcome in "MEMP_post1", "MEMP_post2", "MEMP_post3", "MEMP_post4", "REGEMP_post1", "REGEMP_post2", "REGEMP_post3", "REGEMP_post4":
        result = func_select_outcome_df(outcome=outcome, results_csv="OUT_LABOR_HETEROGENEITY.csv")
        coefplot(data=result, outcome=outcome,  tick_size=20, label_size=20, xlim=(-0.2,0.2), appendix=True)
        plt.close()

    # Earnings (annual) marginal
    for outcome in "MEMP_earnings_post1", "MEMP_earnings_post2", "MEMP_earnings_post3", "MEMP_earnings_post4":
        result = func_select_outcome_df(outcome=outcome, results_csv="OUT_LABOR_HETEROGENEITY.csv")
        coefplot(data=result, outcome=outcome,  tick_size=20, label_size=20, xlim=(-500,500), appendix=True)
        plt.close()

    # Earnings (annual) insured
    for outcome in "REGEMP_earnings_post1", "REGEMP_earnings_post2", "REGEMP_earnings_post3", "REGEMP_earnings_post4":
        result = func_select_outcome_df(outcome=outcome, results_csv="OUT_LABOR_HETEROGENEITY.csv")
        coefplot(data=result, outcome=outcome,  tick_size=20, label_size=20, xlim=(-3000,3000), appendix=True)
        plt.close()
