...
def extract_covariates_table(input_csv_path: str, output_tex_path: str) -> pd.DataFrame:
    ...
    tab["coef"] = np.where(
        tab["pvalue"] < 0.05, tab["coef"].astype(str) + "*", tab["coef"].astype(str)
    )
    tab["coef"] = np.where(
        tab["pvalue"] < 0.01, tab["coef"].astype(str) + "*", tab["coef"].astype(str)
    )
    tab["coef"] = np.where(
        tab["pvalue"] < 0.001, tab["coef"].astype(str) + "*", tab["coef"].astype(str)
    )
    ...
    out.columns = [
        "Estimates (SE)",
        "Control mean [BW]",
        "Percent of bandwidths w/ (p<0.05)",
    ]
