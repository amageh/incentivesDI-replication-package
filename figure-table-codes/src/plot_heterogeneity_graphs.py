...
def coefplot(
    ...
    # Get data for plot.
    if relative:
        coefficients = np.flip(data["Coefficent"].to_numpy()) / np.flip(
            data["ControlMean"].to_numpy()
        )
        std_errors = np.flip(data["StandardError"].to_numpy()) / np.flip(
            data["ControlMean"].to_numpy()
        )
    else:
    ...
    if relative:
        plt.savefig(
            f"../out/{prfx}rdd-heterogeneity-{outcome}-relative.eps",
            dpi=300,
            bbox_inches="tight",
            format="eps",
        )
        ...
        plt.savefig(
            f"../out/{prfx}rdd-heterogeneity-{outcome}.eps",
            dpi=300,
            bbox_inches="tight",
            format="eps",
        )
...
def func_select_outcome_df(outcome: str, results_csv: str):
    ...
    empties[:] = " "
    out = pd.concat(
        [out, pd.DataFrame(empties, index=list(range(10001, 10006)))], axis=1
    )
    out["id"] = [
        1,
        3,
