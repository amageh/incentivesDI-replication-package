...
* Purpose: RDD estimates bw sensitivity mortality
********************************************************************************

* Set path for outputs
local path = "$PROCESS_TEMP/D_temp_mortality/"
local path_out = "$D_OUT_MORTALITY/"
...
clear 
local path = "$PROCESS_TEMP/D_temp_mortality/"
foreach g in $Y_mortality {
	
	append using "`path'/MAT_`g'.dta"
}

export delimited using "$D_OUT_MORTALITY/OUT_RDD_MORTALITY_BW.csv", replace
save "`path'/OUT_RDD_MORTALITY.dta", replace
