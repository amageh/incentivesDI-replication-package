...
* Purpose: RDD estimates for DI status/exit
********************************************************************************
...
foreach g in aall age_1 age_2 age_3 female male diag_1 diag_2 diag_3 diag_4 diag_5 diag_6 quint1 quint2 quint3 quint4 quint5 occ_service occ_manufact occ_technic occ_other {
	...
	********************************************************************************
	...
	matrix list MAT
	matsave MAT, replace p($D_TEMP_STATUS/) dropall
	use $D_TEMP_STATUS/MAT.dta, replace
	...
	* rename vars
	...
	drop _rowname
	gen subsample = "`g'"
	save $D_TEMP_STATUS/MAT_`g'.dta, replace
}

use $D_TEMP_STATUS/MAT_aall.dta, clear 
...
foreach g in age_1 age_2 age_3 female male diag_1 diag_2 diag_3 diag_4 diag_5 diag_6 quint1 quint2 quint3 quint4 quint5 occ_service occ_manufact occ_technic occ_other {
	...
	append using $D_TEMP_STATUS/MAT_`g'.dta

}

export delimited using $D_OUT_STATUS/OUT_STATUS_HETEROGENEITY.csv, replace

save $D_TEMP_STATUS/MAT_ALL.dta, replace
