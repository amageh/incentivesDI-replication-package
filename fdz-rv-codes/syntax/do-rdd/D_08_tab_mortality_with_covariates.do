...
* Purpose: RDD estimates mortality with covariates
********************************************************************************
...
foreach g in aall female male {
	...
	********************************************************************************
	...
	matrix list MAT
	matsave MAT, replace p($D_TEMP_MORTALITY/with_controls/) dropall
	use $D_TEMP_MORTALITY/with_controls/MAT.dta, replace
	...
	* rename vars
	...
	gen subsample = "`g'"
	drop _rowname
	save $D_TEMP_MORTALITY/with_controls/MAT_`g'.dta, replace


use $D_TEMP_MORTALITY/with_controls/MAT_aall.dta, clear 

foreach g in female male  {
	
	append using $D_TEMP_MORTALITY/with_controls/MAT_`g'.dta

}
export delimited using $D_OUT_MORTALITY/OUT_MORTALITY_CONTROLS.csv, replace

save $D_TEMP_MORTALITY/with_controls/MAT_ALL.dta, replace
